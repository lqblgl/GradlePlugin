...
import com.branch.dellog.DelLogPlugin
import com.branch.dellog.PluginExtension
...
public class CleanTask extends DefaultTask {
    ...
    public CleanTask() {
        group = DelLogPlugin.GROUP
        description = "Removes unused resources reported by Android lint task"
    }

    @TaskAction
    def start() {
        def ext = project.extensions.findByName(DelLogPlugin.EXTENSION_NAME) as PluginExtension
    ...
    def isDelFile(String path) {
        String dir = path
        (dir.contains('drawable') || dir.contains('mipmap') || dir.contains('menu')) && (dir.endsWith('.png') || dir.endsWith('.jpg') || dir.endsWith('.jpeg'))
    }
